...
class Fields {
            ...
            if ($this->CI->form_validation->run() === true) {
                if ($method == 'new') {
                    ...
                    if (!$result_id = $this->CI->row_m->update_entry(
                            $stream_fields, $stream, $row->id, $this->CI->input->post(), $skips
                    )) {
    ...
    public function run_field_events($stream_fields, $skips = array(), $values = array()) {
        if (!$stream_fields or (!is_array($stream_fields) and !is_object($stream_fields)))
            return null;
        ...
        foreach ($stream_fields as $field) {
            ...
            // Set the value
            if (isset($values[$field->field_slug]))
                $field->value = $values[$field->field_slug];
    ...
    public function set_rules($stream_fields, $method, $skips = array(), $return_array = false, $row_id = null) {
        if (!$stream_fields or !is_object($stream_fields))
            return array();
