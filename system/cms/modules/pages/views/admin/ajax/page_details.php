...
<div class="buttons">
	<?php 
		...
		{
			echo anchor('admin/pages/choose_type?modal=true&parent='.$page->id, lang('pages:create_label'), 'class="btn blue"');
		}
		else
		{
			$type_id = $this->db->select('id')->limit(1)->get('page_types')->row()->id;
			echo anchor('admin/pages/create?parent='.$page->id.'&page_type='.$type_id, lang('pages:create_label'), 'class="btn green"');
		}

	?>
	<?php echo anchor('admin/pages/duplicate/'.$page->id, lang('pages:duplicate_label'), 'class="btn blue"') ?>
	<?php echo anchor('admin/pages/edit/'.$page->id, lang('global:edit'), 'class="btn blue"') ?>
	<?php echo anchor('admin/pages/delete/'.$page->id, lang('global:delete'), 'class="btn red"') ?>
</div>